---
title: "`{bidux}` v0.3.2: Leaner, Quieter, More Intuitive BID Workflows"
subtitle: 'The `{bidux}` v0.3.2 release streamlines BID workflows with intuitive flat APIs, built-in telemetry presets, and a quieter, leaner experience, helping you focus on insights rather than overhead.'
date: '2025-10-28'
date-format: "MMMM D, YYYY"
categories: ["Software Development", "User Experience"]
tags: ["r", "shiny", "package-release", "behavioral-science", "user-experience"]
image: featured.png
toc: true
citation: true
aliases:
  - /blog/leaner-quieter-intuitive
---
...
Crafting a BID data story in **v0.3.2** is more intuitive. The `new_data_story()` function now uses a **flat API** with four straightforward arguments (`hook`, `context`, `tension`, `resolution`), instead of nesting parts of the story inside.
...
If you've been ingesting telemetry data to inform your designs, `{bidux}` v0.3.2 introduces a welcome convenience: **telemetry sensitivity presets**.
...
Do you ever feel inundated by informational messages when running BID analyses or pipelines? In v0.3.2, quiet mode is here to help.
...
Upgrading to v0.3.2 should be seamless. All your code from v0.3.1 (and even v0.3.0) will continue to run, thanks to careful backward compatibility. If you do use any now-deprecated patterns, bidux will politely prod you with warnings and guidance:
  ...
  - **Legacy telemetry ingestion:** The old `bid_ingest_telemetry()` function remains available for those who haven't migrated to the new `bid_telemetry()` workflow. It continues to return the hybrid list-like object as before. However, we signaled in v0.3.1 that a shift toward `bid_telemetry()` is the future, and that remains true. Plan to update to the tidy tibble approach when convenient.
...
In short, **no breaking changes** in v0.3.2. We've laid the groundwork for 0.4.0 deprecations but provided a smooth on-ramp.
...
You can install `{bidux}` **v0.3.2** from CRAN today:

```{r, eval=FALSE}
install.packages("bidux")
```

As a minor release after v0.3.1, this version focuses on refining your experience: Making the API more intuitive, reducing friction from extraneous output, and keeping the package lean and efficient.
